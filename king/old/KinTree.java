...
//##################################################################################################
    private void initActions()
    {
        ...
        acVisible       = new ReflectiveAction("Show/Hide", null, this, "onToggleVisibility");
            ...
            acVisible.bind(tree);
        acCut           = new ReflectiveAction("Cut", prefs.cutIcon, this, "onCopy");
        ...
        acPaste         = new ReflectiveAction("Paste", prefs.pasteIcon, this, "onPaste");
            acPaste.setTooltip("Re-insert the last object that was Cut from the hierarchy");
    ...
    public void onDelete(ActionEvent ev)
    {
        ...
        if(model != null)
        {
            if(!kMain.prefs.getBoolean("treeConfirmDelete") ||
               ...
               "Confirm delete", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE) == JOptionPane.YES_OPTION)
            {
                model.removeNodeFromParent(node); // updates tree
                ((AGE)node).setOwner(null); // ensure the child knows too
            }
        }
...
//##################################################################################################
    ...
    public void onPaste(ActionEvent ev)
    {
        ...
        if(pasteImpl(path, node, clipboard))
        {
            clipboard = null;
        }
        else
...
//{{{ onCopy
//##################################################################################################
    /** Duplicates the selected group, subgroup, or list. */
    // This method is the target of reflection -- DO NOT CHANGE ITS NAME
    public void onCopy(ActionEvent ev)
    {
        ...
        MutableTreeNode node = (MutableTreeNode)path.getLastPathComponent();
        if(node == null || node instanceof Kinemage || !(node instanceof AGE)) return;
        
        try
        {
            AGE child = (AGE) node;
            AGE clone = (AGE) child.clone();
            AGE parent = child.getOwner();
            pasteImpl(path.getParentPath(), parent, clone);
        }
...
//##################################################################################################
    ...
    */
    boolean pasteImpl(TreePath path, MutableTreeNode insertInto, MutableTreeNode toInsert)
    {
        DefaultTreeModel model = (DefaultTreeModel)tree.getModel();
        if(model == null) return false;
        
        boolean succeeded = false;
        if(insertInto instanceof Kinemage)
        {
            Kinemage kin = (Kinemage)insertInto;
            if(toInsert instanceof KGroup)
            {
                model.insertNodeInto(toInsert, kin, 0);
                toInsert.setParent(kin);
                tree.setSelectionPath(path.pathByAddingChild(toInsert));
                succeeded = true;
            }
            else if(toInsert instanceof KSubgroup)
            {
                KGroup group = new KGroup(kin, "New group");
                model.insertNodeInto(group, insertInto, 0);
            ...
            else if(toInsert instanceof KList)
            {
                KGroup group = new KGroup(kin, "New group");
                model.insertNodeInto(group, insertInto, 0);
                path = path.pathByAddingChild(group);
        ...
        }//insertInto is Kinemage
        else if(insertInto instanceof KGroup)
        {
            KGroup group = (KGroup)insertInto;
            ...
            {
                KSubgroup toInsert;
                for(Iterator iter = ((AGE)toInsert).iterator(); iter.hasNext(); )
                {
        ...
        }//insertInto is KGroup
        else if(insertInto instanceof KSubgroup)
        {
            KSubgroup subgroup = (KSubgroup)insertInto;
            
            if(toInsert instanceof KSubgroup &&
               (!kMain.prefs.getBoolean("treeConfirmMerge") ||
        ...
        }//insertInto is KSubgroup
        else if(insertInto instanceof KList)
        {
            KList list = (KList)insertInto;
            
            if(toInsert instanceof KList)
            {
                KList toMerge = (KList)toInsert;
