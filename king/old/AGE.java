...
//##################################################################################################
    ...
    public Container buildButtons(Container cont)
    {
        if(hasButton()) cont.add(cbox);
        if(!isDominant())
        {
            // We use a JPanel instead of a Box because Boxes in
            // Java 1.3 can't have borders, which makes life difficult.
            JPanel subbox = new JPanel();
            subbox.setLayout(new BoxLayout(subbox, BoxLayout.Y_AXIS));
            subbox.setBorder(BorderFactory.createEmptyBorder(0, 6, 0, 0));
