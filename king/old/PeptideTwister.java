...
//}}}
/**
* <code>PeptideTwister</code> is an interface that allows one
...
*/
public class PeptideTwister extends JPanel implements ActionListener
{
//{{{ Constants
//}}}
...
//##################################################################################################
    ...
    * Constructor
    */
    public PeptideTwister(CairoTool parent, Collection res)
    {
...
//##################################################################################################
    private void buildGUI()
    {
        ...
        resLabels   = new JLabel[    residues.length   ][ 4 ];
        ...
        enableDials = new JCheckBox("Enable peptide rotation and C-beta idealization", false);
    ...
    private Component makeHeaderLabels()
    {
        JLabel[] headerLabels = new JLabel[] { new JLabel("Residue"), 
            new JLabel("Tau dev"), new JLabel("Ramachdrn"), new JLabel("phi,psi") };
        Box box = Box.createVerticalBox();
...
//##################################################################################################
    ModelState updateConformation(ModelState startState)
    {
        if(enableDials.isSelected())
        {
            ...
            return CaRotation.twistPeptides(residues, startState, angles);
        }
