...
//##################################################################################################
    ...
    */
    public static Collection makeMobileGroup(Model model, Residue r1, Residue r2)
    {
        // who's on first?
        Residue first, last;
        ...
        residues.add(first);
        Residue next;
        for(next = first.getNext(model); next != null && !next.equals(last); next = next.getNext(model))
        {
            residues.add(next);
        }
        if(next == null)
...
//##################################################################################################
    ...
    */
    public static ModelState makeConformation(Collection residues, ModelState state, double theta, boolean idealizeCB)
    {
