...
//##################################################################################################
    ...
    */
    public double valueAt(double[] pt)
    {
        ...
        double[] home_ctr = centerOf(home); // center of the above bin (nearest)
        ...
        double[] contrib = new double[nDim]; // fractional contribution of neighbor[] to value at pt[]
        double coeff;                       // product of a mix of elements from contrib[] and (1-contrib[])
        ...
        for(dim = 0; dim < nDim; dim++)
        {
            ...
            contrib[dim] = Math.abs( (pt[dim]-home_ctr[dim])/wBin[dim] ); // always on [0.0, 0.5]
        }
...
//##################################################################################################
    ...
    */
    public void tallyGaussian(double[] pt, double mask)
    {
        tgf_b = mask / 1.5174; // sqrt[ -ln .10 ] = 1.5147
        tgf_a = 1.0 / Math.pow(tgf_b * 1.7725, nDim); // sqrt(pi) = 1.7725
...
//##################################################################################################
    ...
    */
    public void tallyGaussian(double[] pt, double mask, double scale)
    {
        realcount++;
    ...
    void tgfRecursiveLoop(int depth)
    {
        ...
        else
        {
            ...
            if(i != -1)
            {
                ...
                double dist_2 = distanceSquared(bin_center, tgf_pt);
...
//##################################################################################################
    ...
    */
    public void tallyCosine(double[] pt, double mask)
    {
        double scale = 1.0 / Math.pow(mask, nDim);
...
//##################################################################################################
    ...
    */
    public void tallyCosine(double[] pt, double mask, double scale)
    {
        realcount++;
    ...
    void cosRecursiveLoop(int depth)
    {
        ...
        else
        {
            ...
            if(i != -1)
            {
                ...
                {
                    dist = Math.sqrt(dist);
                    //double cosine = Math.max(0, tgf_a * Math.cos( dist/tgf_b ));
...
//##################################################################################################
    ...
    */
    public NDimTable(DataInputStream in) throws IOException
    {
        // Extract our name
        ...
        minVal  = new   double[nDim];
        maxVal  = new   double[nDim];
        nBins   = new     int[nDim];
