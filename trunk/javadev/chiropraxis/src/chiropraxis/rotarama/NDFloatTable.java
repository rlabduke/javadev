...
//##################################################################################################
    ...
    float[] va_home_ctr;    // center of the above bin (nearest)
    ...
    float[] va_contrib;     // fractional contribution of neighbor[] to value at pt[]
//}}}
...
//##################################################################################################
    ...
    */
    public NDFloatTable(String name, int ndim, float[] min, float[] max, int[] bins, boolean[] wrap)
    {
        ...
        va_home_ctr = new float[nDim];
        va_neighbor = new int[nDim];
        va_current  = new int[nDim];
        va_contrib  = new float[nDim];
...
//##################################################################################################
    ...
    */
    public float valueAt(float[] pt)
    {
        ...
        float   coeff;                  // product of a mix of elements from va_contrib[] and (1-va_contrib[])
        ...
        for(dim = 0; dim < nDim; dim++)
        {
            ...
            va_contrib[dim] = (float)Math.abs( (pt[dim]-va_home_ctr[dim])/wBin[dim] ); // always on [0.0, 0.5]
        }
...
//##################################################################################################
    ...
    */
    ...
    public void whereIs(float[] pt, int[] where)
    {
        for(int i = 0; i < nDim; i++)
        {
            // min() is necessary for the case where pt[i] == max[i]
    ...
    */
    ...
    public void centerOf(int[] where, float[] pt)
    {
        for(int i = 0; i < nDim; i++)
        {
            pt[i] = minVal[i] + wBin[i]*((float)where[i]+0.5f);
...
//##################################################################################################
    ...
    */
    public NDFloatTable(DataInputStream in) throws IOException
    {
        ...
        // Allocate table and initialize it
        lookupTable = new float[n_entries];
        //realcount = 0; Must have been a mistake!
