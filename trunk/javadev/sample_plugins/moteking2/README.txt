README FOR WIIREMOTEJ LIBRARY v1.0
6/25/07

Welcome to WiiRemoteJ! At long long last (and I'm very sorry about that), WiiRemoteJ 1.0 is ready for the public. Isn't that exciting, eh? Alright, so there are some things you should know. Let's proceed.
...
ABOUT JSR082
The JSR082 API provides an interface for Java to access the Bluetooth hardware on your system. At the time of this writing, there are very few implementations available. The only one that I have seen any success with is Avetana Bluetooth, and it has a pricetag (not a very big one, but it has one nonetheless). Luckily for Linux users, there is a free implementation of Avetana Bluetooth. Mac and Windows users will have to pay. Note for Windows users: you will need to use the WIDCOMM Bluetooth stack, so make sure your Bluetooth hardware is compatible. It is your responsibility to find a copy of JSR082. Don't ask me for help in that area.
...
To execute your application with WiiRemoteJ in the classpath, type the following: javac -cp PATH/TO/YOUR/CLASSES/;PATH/TO/WiiRemoteJ.jar
...
CONNECTING A WIIREMOTE
In case it isn't clear, this is how you connect your Wii Remote: 
1. Call one of the findRemote methods.
2. Push the 1 and 2 buttons on your Wii Remote simultaneously or press the red button under the battery case.
...
Interleaved input is implemented, but has not been tested. The IR part of interleaved input is not implemented, as it is not yet fully understood by the community (or me).

ADPCM sound format for speaker playback is implemented, but has not been tested.

The scroll wheel directions are reversed on the Mac. This is a fault of Apple's implementation and will hopefully be fixed in Leopard.
