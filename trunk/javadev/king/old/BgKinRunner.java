...
<ul>
<li><b>{0}</b> the full path and file name for the "base" PDB file</li>
<li><b>{1}</b> a comma-separated list of residue numbers for molten residues</li>
<li><b>{2} <font color='#ff0000'>(DEPRECATED)</font></b> a bar-separated list of residue numbers for molten residues</li>
<li><b>{3}</b> the center of view for the current kinemage</li>
<li><b>{4}</b> </li>
<li><b>{5}</b> </li>
<li><b>{6}</b> </li>
<li><b>{7}</b> </li>
...
//##################################################################################################
    ...
    /** The command string with placeholders ({0}, {1}, etc.) intact. */
    ...
    volatile boolean        dumpCmdLine         = false;
    volatile boolean        dumpStdErr          = false;
    volatile boolean        dumpStdOut          = false;
//}}}
...
//##################################################################################################
    ...
    void runCommand(Collection residues, ModelState state, File pdbfile, String cmdTplt)
        throws IOException
    {
        // Build replacement strings for {0}, {1}, etc.
        // {1} and {2}       
        StringBuffer resCommas = new StringBuffer();
        StringBuffer resBars = new StringBuffer();
        for(Iterator iter = residues.iterator(); iter.hasNext(); )
        {
            ...
            if(resBars.length() > 0) resBars.append("|");
            resBars.append(res.getSequenceNumber());
        }
        // {3}
        ...
        String cmdLine = Strings.expandVariables(cmdTplt, cmdParams);
        if(dumpCmdLine)
        ...
        if(!tank.fillTank(proc, 5000)) // 5 sec timeout
...
//##################################################################################################
    ...
    */
    public boolean editCommand(Component dlgParent)
    {
        ...
        Object[] msg = {
            "{0} is the full path to the PDB file",
            "{1} is a list of molten residues: 1,2,3",
            "{2} is a list of molten residues: 1|2|3",
            "{3} is the current center of view: x, y, z",
        };
